...
const NumberButton = (props: NumberButtonProps): JSX.Element => {
  ...
  return (
    <>
      <div className="flex flex-col items-center justify-center p-4 pt-3 w-[156px] lg:w-64 border-2 border-gray-600">
        <div className="mb-4 text-lg lg:text-2xl">{text?.()}</div>
        <button
          type="button"
          onClick={incrementValue}
          className={`h-14 w-20 text-3xl rounded-t-lg border-4 flex items-center justify-center ${props.className}`}
        >
          +
        </button>
        <input
        ...
        <button
          ...
          className={`h-14 w-20 text-3xl rounded-b-lg border-4 flex items-center justify-center ${props.className}`}
        >
          -
        </button>
      </div>
